"""
WO Approval Audit Formatter v12.5 — Fixed WOLI linkage for destination gate.
Changes from v12.4:
  - Destination gate now uses Work Order Line Item sheet (SF ID join) as primary source
  - Falls back to legacy WOLI sheet only for WOs not in Work Order Line Item
  - Fixes 19 WOs that showed 'No labor' or 'No WOLI lines' due to broken linkage
  - Travel home inferred from chronological position (travel after labor = return)
...
def dq_gate(cause_raw, ca_raw, subtype='', wo_num='', has_parts=False):
    ...
    blocked_reasons = list(auto_fails)
    if score_100 < DQ_BLOCK_THRESHOLD:
        blocked_reasons.append(f"Score {score_100}/100 below threshold {DQ_BLOCK_THRESHOLD}")
    score_dict = {
    ...
    if blocked_reasons:
        detail = f"Grade {grade} ({score_100}/100) | BLOCKED: {'; '.join(blocked_reasons[:2])}{warn_str}"
...
def build_destination_lookups(data):
    ...
    return woli_lookup, sa_lookup, wo_customer
...
def gate_destination(wrow, woli_lookup, sa_lookup, wo_customer):
    """
    ...
    if not dates_with_labor:
        return "Fail", "No labor logged — 0 destination fees apply"
    ...
    # ── Build result ─────────────────────────────────────────────────────────
    if remove_dates or no_fee_dates:
        parts = []
        if fees_this_wo:
            parts.append(f"{fees_this_wo} destination fee(s) valid")
        if remove_dates:
            parts.append("remove fee: " + ", ".join(remove_dates[:2]))
        if no_fee_dates:
            parts.append("no fee justified: " + ", ".join(str(d) for d in no_fee_dates[:2]))
        return "Fail", " | ".join(parts)

    return "Pass", f"{fees_this_wo} destination fee(s) — {len(dates_with_labor)} visit date(s)"
...
def gate_ro_coverage(parts_for_wo, wst, ro_df, wo_num):
    ...
    for p in parts_for_wo:
        ...
        _, _, note = audit(p, wst, ro_lines)
        if note and any(x in note for x in ("Missing","No RO","Wrong RO","needs NNU","no RO")):
            issues.append(f"{pn}: {note}")
        elif note:
            ok.append(f"{pn}: {note}")
        else:
...
def build_gate_summary(ws, data):
    ...
    woli_lk, sa_lk, wo_cust = build_destination_lookups(data)
    ...
    for _, wrow in wo_df.iterrows():
        ...
        gates = [
            ...
            ("Destination Review",           gate_destination(wrow.to_dict(), woli_lk, sa_lk, wo_cust)),
...
# Accepts file-like objects (BytesIO / UploadedFile), returns workbook bytes.
# ══════════════════════════════════════════════════════════════════════════════
...
def run_audit(wo_file, haas_file=None):
    ...
    dict with keys:
        ...
        'gate_summary': dict — {pass: int, fail: int, warn: int} from gate sheet
    """
    import io
    log = []
    ...
    if "WO Gate Summary" in wb.sheetnames:
        wb.move_sheet("WO Gate Summary", offset=-(len(wb.sheetnames)-2))
    ...
    try:
        ...
        for _, wrow in wo_df.iterrows():
            ...
            gates_results = [
                gate_wo_status(wrow.to_dict())[0],
                gate_destination(wrow.to_dict(), woli_lk, sa_lk, wo_cust)[0],
                gate_ro_eligible_parts(parts)[0],
                gate_pr_prli(parts)[0],
                gate_consumed_vs_nnu(parts)[0],
                gate_ro_coverage(parts, sub, ro_df, wn)[0],
                gate_ro_status(parts, ro_df, wn)[0],
                dq_gate(cause, ca, sub, wn, has_parts=len(parts)>0)[0],
            ]
            if any(r == "Fail" for r in gates_results):
                gate_summary["fail"] += 1
            elif any(r == "Warn" for r in gates_results):
                gate_summary["warn"] += 1
            else:
                gate_summary["pass"] += 1
    except Exception:
    ...
    if built:
        wb.save(buf)
        _log(f"Sheets built: {len(built)}/{len(built)+len(failed)} \u2014 {', '.join(built)}")
    else:
