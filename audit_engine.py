"""
WO Approval Audit Formatter v12.6 — Gate refinements & feedback loop.
Changes from v12.5:
...
def gate_destination(wrow, woli_lookup, sa_lookup, wo_customer):
    ...
    if fees_apply:
        summary += f" | {fees_apply} destination fee(s) apply"
    if under_threshold:
    ...
    try:
        ...
        for _, wrow in wo_df.iterrows():
            ...
            gate_results = [
                ...
                ("Required RO Coverage",         gate_ro_coverage(parts, sub, ro_df, wn)),
                ("RO Status Valid",              gate_ro_status(parts, ro_df, wn)),
                ("Documentation Quality",        dq_gate(cause, ca, sub, wn, has_parts=wo_has_parts)[:2]),
            ]
